...
class Train(object):
    ...
    def get_action_from_obs(self,obs_target, arm_down):
        ...
        if abs(obs_target[2]-obs_target[0])<0.5: # local: check if bbox not too big
        ...
        elif 0.30>=dist>0.08 or (dist>0.30 and arm_down==1) or (dist<=0.08 and (error[0]>0.035 or error[0]<-0.035)): # arm down
