...
def get_image_from_cam():
    ...
    try:
        while True:
            ...
            ## TODO: normalization
            Result_arm = model(color_image_arm)
            Result_base = model(color_image_base) 
            bbox_arm = Result_arm.boxes.xyxy[0].cpu().numpy().astype(int)
            bbox_base = Result_base.boxes.xyxy[0].cpu().numpy().astype(int)
            # bbox = agent.sample(state/255.0)

            bboxs = bbox_arm + bbox_base
            if count%2==0:
                s = time.time()
            ...
            box1 = np.rint([bbox_arm[0]*1280, bbox_arm[1]*720, bbox_arm[2]*1280, bbox_arm[3]*720])
            box2 = np.rint([bbox_base[0]*1280, bbox_base[1]*720, bbox_base[2]*1280, bbox_base[3]*720])
