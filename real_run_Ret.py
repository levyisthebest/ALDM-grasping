...
world_target_global = np.array([0.6953, 0.6944])
...
class Train(object):
    ...
    def get_action_from_obs(self,obs_target, arm_down):
        ...
        if arm_down==1: # switch from global to local
            error = [0.01*error_1[0]+0.99*error_2[0], error_1[1]]
        else:
        ...
        elif 0.30>=dist>0.085 or (dist>0.30 and arm_down==1) or (dist<=0.085 and (error[0]>0.035 or error[0]<-0.035)): # arm down
